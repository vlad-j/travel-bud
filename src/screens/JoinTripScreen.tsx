...
export default function JoinTripScreen() {
  ...
  async function handleJoin() {
    ...
    try {
      ...
      if (data?.error) { Alert.alert('Could not join', data.error); setLoading(false); return; }
      ...
      if (!tripId) { Alert.alert('Error', 'Something went wrong.'); setLoading(false); return; }
      ...
      Alert.alert(
        ...
        alreadyMember
          ...
          : `Welcome to "${tripName}"! You joined as ${role === 'owner' ? '👑 Owner' : '✏️ Editor'}.`,
        [{ text: 'Open Trip', onPress: () => navigation.reset({ index: 0, routes: [{ name: 'Home' }] }) }]
      );
    } catch (e: any) {
...
const styles = StyleSheet.create({
  ...
  flex: { flex: 1 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#F0F0F0' },
  ...
  codeInput: { width: '100%', textAlign: 'center', fontSize: 32, fontWeight: '900', color: '#1A1A1A', backgroundColor: '#F5F5F5', borderRadius: 16, paddingVertical: 18, paddingHorizontal: 20, borderWidth: 2, borderColor: '#E0E0E0', letterSpacing: 8 },
  hint: { fontSize: 12, color: '#BBB', marginBottom: 32 },
  joinBtn: { width: '100%', backgroundColor: '#4CAF50', borderRadius: 16, paddingVertical: 16, alignItems: 'center', marginBottom: 12 },
